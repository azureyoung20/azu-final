...
from progress.data.image_producer import GridImageDataset  # noqa
from progress.densenet_crf import MODELS  # noqa
# Set this option to avoid "IOError: image file is truncated" exceptions
...
def valid_epoch(summary, cfg, model, loss_fn,
    ...
    for step in range(steps):
        ...
        batch_size, num_patches, num_channels, height, width = data.shape
        data = data.view(batch_size * num_patches, num_channels, height, width)

        output = model(data)
        output = output.view(batch_size, num_patches, -1)
        target = target.view(batch_size, num_patches, -1)
        loss = loss_fn(output, target)
